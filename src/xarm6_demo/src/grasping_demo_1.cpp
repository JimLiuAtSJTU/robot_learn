...
{	
	...
	if ( marker_tmp.pose.position.x > -0.2 && marker_tmp.pose.position.x < 1.0 && marker_tmp.pose.position.y > 0.0 && marker_tmp.pose.position.y < 1.0 && 
...
void attainPosition (double po_x, double po_y, double po_z, vector<double> ori)
{	
	// 该函数计算物体在机械臂坐标系下的位置(TCP_offset)
	
	Eigen::Quaterniond quaternion4( ori[3], ori[0], ori[1], ori[2]);// 机械臂rpy 四元数
	...
	Transform << rotation_matrix3(0,0), rotation_matrix3(0,1), rotation_matrix3(0,2), po_x,
		  rotation_matrix3(1,0), rotation_matrix3(1,1), rotation_matrix3(1,2), po_y, 
		  rotation_matrix3(2,0), rotation_matrix3(2,1), rotation_matrix3(2,2), po_z,
		  0.0,		   0.0,		    0.0,		     1.0;
...
int main(int argc, char **argv)
{
	...
	joint_group_positions[1] = -41.3/180.0*M_PI;
	joint_group_positions[2] = 1.6/180.0*M_PI;
	joint_group_positions[3] = 0.0/180.0*M_PI;
	joint_group_positions[4] = -46.1/180.0*M_PI;
	...
	double doorOpenAngle = dooreulerAngle4[0]; // 门打开的角度
	cout << "********************************************************" << doorOpenAngle << endl;
	std::vector<double> kkk(3);
	kkk[0] = 90.0/180.0*M_PI;
	kkk[1] = 0.0/180.0*M_PI;
	kkk[2] = 90.0/180.0*M_PI + doorOpenAngle;
	cout << "**********************************************" << kkk[2] <<endl;

	Eigen::Quaterniond quaternion1;
    	quaternion1 = Eigen::AngleAxisd(kkk[2], Eigen::Vector3d::UnitZ()) * 
           	   Eigen::AngleAxisd(kkk[1], Eigen::Vector3d::UnitY()) * 
           	   Eigen::AngleAxisd(kkk[0], Eigen::Vector3d::UnitX());
	cout << "quaternion1 x: " << quaternion1.x() << endl;
	// 机械臂运动到门把手的位置

	if (ID == 10)		// 若Marker的id为10	
	{	
		// 仓库门的开门姿势
		std::vector<double> orientation(4);
		Marker_pose.pose.orientation.x = orientation[0] = 0.5;  
		Marker_pose.pose.orientation.y = orientation[1] = 0.5;	 
		Marker_pose.pose.orientation.z = orientation[2] = 0.5;	 
		Marker_pose.pose.orientation.w = orientation[3] = 0.5;  
		attainPosition (Base_H_Obj[0], Base_H_Obj[1], Base_H_Obj[2], orientation );
	...
	{	
		// 会议室门的开门姿势
		std::vector<double> orientation(4);
		Marker_pose.pose.orientation.x = orientation[0] = quaternion1.x();  
		Marker_pose.pose.orientation.y = orientation[1] = quaternion1.y();	 
		Marker_pose.pose.orientation.z = orientation[2] = quaternion1.z();	 
		Marker_pose.pose.orientation.w = orientation[3] = quaternion1.w(); 
		attainPosition (Base_H_Obj[0], Base_H_Obj[1], Base_H_Obj[2], orientation );
		attainObject (-0.02, 0.0, 0.0);	//输入门把手离标签的xyz的距离
